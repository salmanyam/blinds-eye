...
unsigned char readFile (unsigned char flag, unsigned char *fileName)
{
	...
	unsigned char nam[11];
	
	int samp = 0,samRate[4];
	int sr;
	
	//LCDClear();
	//LCDWriteStringXY(0,1,fileName);
	
	error = convertFileName (nam,fileName); //convert fileName into FAT format
	if(error) 
	{	
		return 2;
	}		
	...
	dir = findFiles (GET_FILE, nam); //get the file location
	if(dir == 0) 
	  return (0);
	...
	/*	if(samp == 0)
		{
			...
			samRate[0] = (int)buffer[28];
			samRate[1] = (int)buffer[27];
			samRate[2] = (int)buffer[31];
			samRate[3] = (int)buffer[30];
			
			sr = conv(samRate);
			
			
			LCDClear();
			LCDWriteIntXY(0,0,sr,8);
			_delay_ms(4000);
		...
		*/
		...
		for(k=0; k<512; k++)
		{
		  //my_data[k] = buffer[k];
			...
			//LCDClear();
			//LCDWriteIntXY(0,0,buffer[k],2)
			 //LCDData(c);
			//LCDWriteIntXY(0,0,c,6);
...
//***************************************************************************
unsigned char convertFileName (unsigned char n[],unsigned char *fileName)
{
